...
router.get('/', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [resources, totalItems] = await Promise.all([
      ...
      Resource.countDocuments(query).setOptions({ strictQuery: false }),
    ]);
...
router.patch('/:id', async (req: AuthRequest, res: Response) => {
  try {
    ...
    res.json({ success: true, message: '게시글이 수정되었습니다.', data: resource });
  } catch (error) {
...
router.post('/:id/restore', async (req: AuthRequest, res: Response) => {
  try {
    ...
    res.json({ success: true, message: '게시글이 복구되었습니다.', data: resource });
  } catch (error) {
...
router.delete('/:id/permanent', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const resource = await Resource.findOneAndDelete({ _id: id, isDeleted: true });
...
router.post('/bulk-permanent-delete', async (req: AuthRequest, res: Response) => {
  try {
    ...
    await Resource.deleteMany({ _id: { $in: ids }, isDeleted: true });
